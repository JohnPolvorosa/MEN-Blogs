...
// Initialize Mongoose
const mongoLocal = `mongodb://localhost:27017/blogsDB`
  ...
  Blog.find({}, function(err, results){
    if (!err) {
      res.render("home", {
        ...
        // Push results (whatever was found) into posts of array
        posts: results
      });
...
app.post("/compose", function(req, res){
  // Grab object from form
  const post = {
    title: req.body.postTitle,
    content: req.body.postBody
  };
  // Create entry into MongoDB
  Blog.create(post, function(err, results) {
    if (!err) {
      console.log(results);
      res.redirect("/");
    } else {
...
app.get("/posts/:postId", function(req, res){
  ...
  Blog.findOne({_id: requestedId}, function(err, results) {
    if (!err) {
      console.log("Found id: ", results);
      res.render("post", {
        title: results.title,
